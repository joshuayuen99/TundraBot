...
export default class YouTube implements Command {
    ...
    category = "fun";
    description =
        "Starts a new YouTube video sharing session for the current voice channel.";
    usage = "youtube";
    enabled = true;
    ...
    slashDescription =
        "Start a new YouTube video sharing session for the current voice channel";
    commandOptions = [];
    ...
    async execute(ctx: CommandContext, args: string[]): Promise<void> {
        ...
        ctx.client.discordTogether
            .createTogetherCode(voice.id, "youtube")
            .then(async (invite) => {
