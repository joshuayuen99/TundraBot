function updateDayOfWeekMessageChart(data) {
    ...
    for (const weekdayID of [1, 2, 3, 4, 5, 6, 7]) {
    ...
    const y = d3
        .scaleLinear()
        .domain([0, d3.max(data, (d) => d.count / messageTotal)])
        .nice(6)
    ...
    // create a tooltip
    var tooltip = d3
        .select("#day-of-week-messages-chart")
        .append("div")
        .style("opacity", 0)
        .attr("class", "tooltip")
    ...
    var mousemove = function (event) {
        ...
        tooltip
            .html(
                `<strong>${this.__data__.dayLong}</strong><br>
                Frequency: ${(
                    (this.__data__.count / messageTotal) *
                    100
                ).toFixed(2)}%<br>
...
function updateWeeklyMessageChart(data) {
    ...
    var x = d3
        .scaleTime()
        .domain([
            moment.utc(Date.now()).subtract(7, "days").startOf("hour"),
            moment.utc(Date.now()).startOf("hour"),
        ])
        .range([0, width]);
    ...
    var yAxis = (g) => g.call(d3.axisLeft(y).ticks(4));
    ...
    svg.append("g").attr("class", "minor-x-axis").call(xMinorAxis);

    // Lines
    svg.append("path").datum(parsedData).attr("class", "area").attr("d", area);

    // Area under the curve
    svg.append("path").datum(parsedData).attr("class", "line").attr("d", line);
    ...
    function mousemove(event) {
        ...
        const selectedData =
            x0 - parsedData[i - 1].date < parsedData[i].date - x0
                ? parsedData[i - 1]
                : parsedData[i];
        focus
        ...
        const minuteString = String(selectedData.date.getMinutes()).padStart(
            2,
            "0"
        );
...
function updateCommandBreakdownChart(test) {
    const container = $("#command-breakdown-chart");
    ...
    var svg = d3
        .select("#command-breakdown-chart")
        .append("svg")
        .style("width", width)
        .style("height", height)
        .append("g")
...
// updateCommandBreakdownChart(null);
